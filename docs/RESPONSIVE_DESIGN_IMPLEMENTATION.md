...
## **PHASE 6: ACCESSIBILITY & TEXT CUTOFF PREVENTION** 
### Additional Requirements for Production

**OBJECTIVE:** Ensure app is fully legible on all devices regardless of resolution, screen density, or accessibility settings. Prevent text cutoff and clipping issues.

### 6.1 Accessibility Font Scaling Support

**Goal:** Respect user's system font size preferences (Settings > Accessibility > Font Size)

**Implementation:**
1. Use `fontScale` from `useWindowDimensions()` or `PixelRatio.getFontScale()`
2. Apply to all font sizes: `fontSize: ms(18) * fontScale`
3. Test with accessibility font sizes: Small, Default, Large, Extra Large, Huge
4. Ensure containers expand to accommodate larger fonts (no fixed heights on text containers)
...
const styles = StyleSheet.create({
  text: {
    fontSize: ms(16) * fontScale,  // Respects user's font size setting
  },
  container: {
    // Don't use fixed height - let it grow with content
    minHeight: ms(48),  // Minimum touch target
    paddingVertical: ms(12),
  }
});
```

### 6.2 Text Cutoff Prevention

**Common Issues:**
- Fixed container heights that clip text
- Missing `numberOfLines` causing overflow
- Horizontal text truncation without ellipsis
- Line height issues causing vertical clipping

**Solutions:**

**A. Remove Fixed Heights on Text Containers**
```typescript
// BAD - Text gets clipped
<View style={{ height: 40 }}>
  <Text>Long prayer name that might wrap</Text>
</View>

// GOOD - Container grows with content
<View style={{ minHeight: ms(40), paddingVertical: ms(8) }}>
  <Text>Long prayer name that might wrap</Text>
</View>
```

**B. Add Ellipsis for Single-Line Text**
```typescript
<Text 
  numberOfLines={1} 
  ellipsizeMode="tail"
...
>
  {mosqueSettings?.mosqueName || 'Al Ansar Masjid'}
</Text>
```

**C. Allow Multi-Line Wrapping Where Appropriate**
```typescript
<Text 
  numberOfLines={2} 
  ellipsizeMode="tail"
...
**D. Use `adjustsFontSizeToFit` for Critical Single-Line Text**
```typescript
<Text 
  adjustsFontSizeToFit 
  numberOfLines={1}
  minimumFontScale={0.8}  // Won't shrink below 80% of original size
...
```typescript
// Use flex instead of fixed widths where possible
<View style={{ flex: 1, flexShrink: 1 }}>  // Allows shrinking if needed
  <Text numberOfLines={1} ellipsizeMode="tail">
    {longText}
  </Text>
</View>

// Add flexWrap for rows that might overflow
...
Test on:
- ✅ Small screen devices (320dp width minimum)
- ✅ Accessibility font sizes (Settings > Display > Font Size > Largest)
- ✅ Long text content (mosque names, event descriptions)
- ✅ Different locales/languages (if applicable)
- ✅ Landscape orientation (where applicable)
- ✅ Split screen/multi-window mode (Android)

**Manual Testing Steps:**
1. Enable largest accessibility font size in device settings
2. Navigate to each screen
3. Check for:
   - Text cutoff (letters missing)
...
1. Prayer Times (Home) — Critical for daily use
2. Events — Long event names/descriptions
3. Donations — Form labels and error messages
4. Qibla — Compass labels and location text
5. Settings — Menu items and descriptions

**For each screen, verify:**
- No fixed heights on text-containing Views
- All interactive Text has proper `numberOfLines` + `ellipsizeMode`
- Containers use `flexShrink: 1` to allow compression
- Font sizes respect `fontScale` multiplier
- Touch targets minimum 48dp (use `TOUCH_TARGET.MINIMUM`)

### 6.6 Implementation Timeline

**Phase 6A: Accessibility Font Support** (2-3 hours)
- Add `fontScale` to useResponsive hook
- Apply to all font size calculations
- Test with max font size setting

**Phase 6B: Text Cutoff Audit & Fix** (3-4 hours)
- Audit all screens for fixed heights
- Add numberOfLines/ellipsizeMode where missing
- Replace fixed heights with minHeight + padding
- Test with long content strings

**Phase 6C: Final Validation** (2 hours)
- Test matrix: 3 devices × 3 font sizes = 9 test scenarios
